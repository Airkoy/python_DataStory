...
功能:
    query_user_list()   - 获取全部用户列表，将 email/uid 输出到 user_list.json
    find_uid_by_email() - 从本地 user_list.json 文件中根据邮箱匹配 uid（纯本地，无需网络）
...
def query_user_list(token: str, params: Dict[str, Any] = None) -> Optional[Dict[str, Any]]:
    """
    获取全部用户列表，提取 email/uid 并输出到 user_list.json。
    ...
    try:
        ...
        print(f"✅ 用户列表已保存到 {USER_LIST_FILE}，共 {len(user_map)} 个用户")
