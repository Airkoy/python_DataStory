...
QUERY_SIZE = 100
QUERY_SORT = [{"publish_timestamp": {"order": "desc"}}]
QUERY_DICT = {
    "bool": {
        "filter": [
        ...
        "must_not": [
            {"terms": {"item_id": ["exclude_1", "exclude_2"]}}
        ],
    }
}
AGG_DICT = {
    "group_by_site": {
        "terms": {"field": "site_id", "size": 10},
        "aggs": {
            "group_by_language": {
                "terms": {"field": "language", "size": 10}
            },
            "site_total_data": {
                "sum": {"field": "interaction_cnt"}
            },
        },
    }
}
...
# ================= 输出结果 =================
save_output(resp)
save_es_query(INDEX_NAME, QUERY_DICT, QUERY_SIZE, sort=None, aggs=AGG_DICT)
