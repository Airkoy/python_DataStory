...
使用方式:
    ...
    2. 运行脚本即可完成充值；可用 --user-list 指定历史缓存
"""
...
def load_user_map(user_list_file: Union[str, Path] = USER_LIST_FILE) -> Dict[str, int]:
    """读取并校验 query_userList.py 生成的用户缓存。"""
    cache_path = Path(user_list_file)
    if not cache_path.exists():
        raise FileNotFoundError(
            f"用户列表文件不存在: {cache_path}，请先运行 query_userList.py"
        )

    with cache_path.open("r", encoding="utf-8") as file:
        user_map = json.load(file)
    if not isinstance(user_map, dict):
        raise ValueError(f"用户列表必须是 email 到 uid 的 JSON 对象: {cache_path}")
    ...
    if metadata_path.exists():
        ...
        generated_at = "未记录"
    print(f"用户缓存: {cache_path} | 生成时间: {generated_at} | 用户数: {len(user_map)}")
    return user_map


def find_uid_by_email(
    email: str,
    user_list_file: Union[str, Path] = USER_LIST_FILE,
) -> Optional[int]:
    """
    ...
    """
    try:
        user_map = load_user_map(user_list_file)
    except (OSError, ValueError, json.JSONDecodeError) as error:
        print(f"❌ 无法读取用户列表: {error}")
        return None
...
def do_charge(
    ...
    resolved_data = resolve_charge_data(charge_data, user_list_file)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="按邮箱或 uid 批量充值")
    parser.add_argument(
        "--user-list",
        type=Path,
        default=USER_LIST_FILE,
        help=f"用户缓存文件（默认: {USER_LIST_FILE}）",
    )
    ...
    CHARGE_DATA: List[Dict[str, Any]] = [
        {
            ...
            "chargeType": 1,
            "number": 60000000,
            "email": "yili.account@datastory.com.cn",
            "remark": "王轶君"
        },
        # 批量充值，可继续添加：
        # {
    ...
    do_charge(CHARGE_DATA, args.user_list)
