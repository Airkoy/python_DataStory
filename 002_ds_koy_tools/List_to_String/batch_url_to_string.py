...
#
# # 示例 1：JSON 数组文件
# # trans_str.json 内容: ["apple", "banana", "cherry"]
# result = json_to_comma_string('trans_str.json')
...
# 示例 3：JSON 数组中含对象
# trans_str.json 内容: [{"id":1,"name":"A"}, {"id":2,"name":"B"}]
# result = json_to_comma_string('trans_str.json')
# print(result)  # 输出: {'id': 1, 'name': 'A'},{'id': 2, 'name': 'B'}

if __name__ == '__main__':
    current_dir = os.path.dirname(os.path.abspath(__file__))
    path = os.path.join(current_dir, 'trans_str.json')
    result = json_to_comma_string(path)
    print(result)
    print("")
    result = ",".join(f'"{x}"' for x in result.split(","))
    print(result)
