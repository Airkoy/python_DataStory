"""
    batch_dcd_urlConvert.py :
        1.在input_url.json中，每行输入1条url
        2.将input_url.json中的url走懂车帝真实转链接口，得到返回的url
        3.将结果写入result_url.json  格式为【 url｜convert_url 】
"""
...
# 读取原始URL列表（每行一个）
with open('input_url.json', 'r', encoding='utf-8') as f:
...
with open('result_url.json', 'w', encoding='utf-8') as f:
    for line in output_lines:
        f.write(line + '\n')

print(f"\n🎉 处理完成！共 {len(output_lines)} 行，结果已保存至 result_url.json")
